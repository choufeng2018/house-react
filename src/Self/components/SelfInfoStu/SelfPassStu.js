...
const {Option} = Select;
class SelfPassStu extends Component{
    ok(){
        const {
            actions: {putOwnerPass, isFresh},
            form:{validateFields}
        } = this.props;
        validateFields( async (err, values) => {
            if (!err) {
                if (values['old_pass'] !== this.state.login_info[0].owner_pass) {
                ...
                let data = {
                    ...
                    owner_pass: values['new_pass'],
                }
                let rst = await putOwnerPass({},data);
                if (rst[0].status === 'ok') {
                        ...
                        <Col offset = {7} span = {8}>
                            <FormItem {...formItemLayout} label='身份证号'>
                                {getFieldDecorator('owner_no',{
                                    ...
                                    initialValue: this.state.login_info[0].owner_no
                                })(
                                    <Input type="text" disabled placeholder = '身份证号'/>
                                )}
                            </FormItem>
                        </Col>
                    </Row>
                        ...
                        <Col offset = {7} span = {8}>
                            <FormItem {...formItemLayout} label='姓名'>
                                {getFieldDecorator('owner_name',{
                                    ...
                                    initialValue: this.state.login_info[0].owner_name
                                })(
...
export default Form.create()(SelfPassStu)

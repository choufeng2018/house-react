...
const {Option} = Select;
class SelfPassRep extends Component{
    ok(){
        const {
            actions: {putRepPass, isFresh},
            form:{validateFields}
        } = this.props;
        validateFields( async (err, values) => {
            if (!err) {
                if (values['old_pass'] !== this.state.login_info[0].repman_pass) {
                ...
                let data = {
                    ...
                    repman_pass: values['new_pass'],
                }
                let rst = await putRepPass({},data);
                if (rst[0].status === 'ok') {
                        ...
                        <Col offset = {7} span = {8}>
                            <FormItem {...formItemLayout} label='工号'>
                                {getFieldDecorator('repman_no',{
                                    ...
                                    initialValue: this.state.login_info[0].repman_no
                                })(
...
export default Form.create()(SelfPassRep)
